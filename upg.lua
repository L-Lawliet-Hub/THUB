...
local function setupAutoExecute()
	if getgenv().AutoExecute and not getgenv().AutoExec then
		...
		queue_on_teleport([[
			...
			task.wait(5)
			getgenv().AutoExec = false
			loadstring(game:HttpGet("https://raw.githubusercontent.com/L-Lawliet-Hub/THUB/main/upg.lua"))()
		]])
	end
end
...
-- [AUTO UPGRADE GEAR]
-- Log-confirmed remote: getRemote:InvokeServer("S_Equipment", "Upgrade", {tbl_of_all_keys})
-- Server accepts ALL upgrade keys at once in one table — no per-key looping.
-- Server returns nothing (n=0), so no return-value check needed.
-- No PlaceId restriction — works in lobby AND in missions/raids.
...
Toggles.AutoUpgradeToggle:OnChanged(function()
	...
	task.spawn(function()
		while getgenv().AutoUpgrade do
			-- fetch live slot data via Data/Copy
			...
			-- collect every stat that hasn't hit max level 15
			local keysToUpgrade = {}
			for upg, lvl in next, upgrades do
				if lvl < 15 then
					table.insert(keysToUpgrade, upg)
				end
			end

			if #keysToUpgrade == 0 then
				-- every stat is already max — stop
			...
			-- single call with the full key table (matches log signature exactly)
			pcall(function()
				getRemote:InvokeServer("S_Equipment", "Upgrade", keysToUpgrade)
			end)

			-- notify a compact summary (server returns nil, assume upgrade fired ok)
			Library:Notify({
				Title = "Upgrading " .. weapon,
				Description = table.concat(keysToUpgrade, ", "):gsub("_", " ") .. " on slot " .. tostring(slotIndex),
				Time = 2,
			})
...
-- [AUTO ENHANCE PERKS]
-- Log-confirmed remote: getRemote:InvokeServer("S_Equipment", "Enhance", equippedPerkId, {food_ids})
-- equippedPerkId = UUID string (NOT wrapped in table).
-- food_ids = table of UUID strings — server accepts up to 20 per call (log shows 20).
-- Server returns nothing (n=0) — do NOT check return value; previous code was always false.
-- Data fetched fresh every iteration from Data/Copy (GetPlayerData returns profile settings, not slots).
-- No PlaceId restriction — works anywhere.
...
Toggles.AutoEnhanceToggle:OnChanged(function()
	...
	task.spawn(function()
		-- grab initial slot data
		local ok, liveData = pcall(function() return getRemote:InvokeServer("Data", "Copy") end)
		if not ok or not liveData or type(liveData) ~= "table" then
			Library:Notify({ Title = "Auto Perk", Description = "Could not load player data.", Time = 3 })
			getgenv().AutoPerk = false
		...
		local slotData  = slotIndex and liveData.Slots and liveData.Slots[slotIndex]
		if not slotData then
			Library:Notify({ Title = "Auto Perk", Description = "No active slot found.", Time = 3 })
			getgenv().AutoPerk = false
			Toggles.AutoEnhanceToggle:SetValue(false)
			return
		end
		...
		local equippedPerkId = slotData.Perks and slotData.Perks.Equipped and slotData.Perks.Equipped[perkSlot]
		if not equippedPerkId or equippedPerkId == "" then
		...
		-- build local storage cache from initial data
		local storagePerks = {}
		if slotData.Perks and slotData.Perks.Storage then
			for id, val in pairs(slotData.Perks.Storage) do
				storagePerks[id] = val
			end
		end

		local equippedPerkData = storagePerks[equippedPerkId]
		if not equippedPerkData then
			Library:Notify({ Title = "Auto Perk", Description = "Equipped perk data not found in storage.", Time = 3 })
			getgenv().AutoPerk = false
			Toggles.AutoEnhanceToggle:SetValue(false)
			return
		end

		local perkName     = equippedPerkData.Name
		local rarity       = GetPerkRarity(perkName)
		local currentLevel = equippedPerkData.Level or 0
		local currentXP    = equippedPerkData.XP    or 0

		while getgenv().AutoPerk do
			-- re-fetch live data each iteration for accurate level/XP tracking
			local ok2, freshData = pcall(function() return getRemote:InvokeServer("Data", "Copy") end)
			if ok2 and freshData and type(freshData) == "table" then
				local freshSlot = freshData.Slots and freshData.Slots[slotIndex]
				if freshSlot and freshSlot.Perks and freshSlot.Perks.Storage then
					-- sync storage from server
					for id, val in pairs(freshSlot.Perks.Storage) do
						if not storagePerks[id] then storagePerks[id] = val end
					end
					-- remove server-deleted perks from local cache
					for id in pairs(storagePerks) do
						if not freshSlot.Perks.Storage[id] then storagePerks[id] = nil end
					end
					-- sync equipped perk level/XP from server
					local freshEquipped = freshSlot.Perks.Storage[equippedPerkId]
					if freshEquipped then
						currentLevel = freshEquipped.Level or currentLevel
						currentXP    = freshEquipped.XP    or currentXP
					end
				end
			end

			if currentLevel >= 10 then
				Library:Notify({ Title = "Auto Perk", Description = perkName .. " is Level 10! Done.", Time = 3 })
				break
			end

			-- build rarity filter from dropdown
			...
			if selectedRarities then
				for r, isActive in pairs(selectedRarities) do
					if isActive then rarityFilter[r] = true end
				end
			end

			-- collect up to 20 food perks (log shows server accepts 20 per call)
			local foodPerks    = {}
			local totalXPGain  = 0
			for perkId, tbl in pairs(storagePerks) do
				if perkId == equippedPerkId then continue end
				local r = GetPerkRarity(tbl.Name)
				if rarityFilter[r] then
					table.insert(foodPerks, perkId)
					totalXPGain = totalXPGain + GetPerkXP(r, math.max(tbl.Level or 0, 1))
					if #foodPerks >= 20 then break end
				end
			end

			if #foodPerks == 0 then
				Library:Notify({ Title = "Auto Perk", Description = "No food perks left in storage.", Time = 3 })
				break
			end

			-- fire Enhance — server returns nothing (n=0); do NOT check return value
			pcall(function()
				getRemote:InvokeServer("S_Equipment", "Enhance", equippedPerkId, foodPerks)
			end)

			-- remove consumed perks from local cache
			for _, id in ipairs(foodPerks) do storagePerks[id] = nil end

			-- update local XP/Level estimate for display
			...
			Library:Notify({
				...
				Description = "~Lv " .. tostring(currentLevel) .. "  (+" .. tostring(totalXPGain) .. " XP, " .. #foodPerks .. " perks fed)",
				Time        = 1.5,
			})
...
-- [AUTO SKILL TREE]
-- Log-confirmed remote: getRemote:InvokeServer("S_Equipment", "Unlock", {"skillId"})
-- skillId = string number wrapped in a single-element table per call.
-- Returns (nil, nil, errorMsg) on failure — check 3rd return for error string.
-- No PlaceId restriction — works anywhere.
...
Toggles.AutoSkillTree:OnChanged(function()
	...
	task.spawn(function()
		while getgenv().AutoSkillTree do
			...
			-- already-unlocked set for fast lookup
			local unlocked = {}
			if slotData.Skills and slotData.Skills.Unlocked then
				for _, id in ipairs(slotData.Skills.Unlocked) do unlocked[id] = true end
			end

			local anyUnlocked = false
			for _, path in ipairs(paths) do
				for _, skillId in ipairs(path) do
					if unlocked[skillId] then continue end

					-- Signature: InvokeServer("S_Equipment", "Unlock", {"skillId"})
					-- Returns (nil, nil, errorMsg) on fail, (nil, nil, nil) on success.
					local pOk, r1, r2, r3 = pcall(function()
						return getRemote:InvokeServer("S_Equipment", "Unlock", {skillId})
					end)

					local errMsg = r3  -- nil = success, string = server error
					if pOk and not errMsg then
						anyUnlocked     = true
						unlocked[skillId] = true  -- mark locally so we don't retry immediately
						Library:Notify({ Title = "Skill Unlocked!", Description = "Node " .. skillId .. " on " .. weapon, Time = 1 })
						task.wait(0.4)
					elseif pOk and errMsg then
						-- server rejected (no gold, wrong order, etc.) — skip this node
						Library:Notify({ Title = "Skill Tree", Description = "Node " .. skillId .. ": " .. tostring(errMsg), Time = 1.5 })
						task.wait(0.3)
					end
				end
			end
			...
			if not anyUnlocked then
				Library:Notify({ Title = "Skill Tree", Description = "All selected paths are complete.", Time = 3 })
				getgenv().AutoSkillTree = false
