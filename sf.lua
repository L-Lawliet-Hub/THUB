...
f targetPart then
    ...
    while currentTitanModel and currentTitanModel.Parent ~= titansFolder do
        currentTitanModel = currentTitanModel.Parent
			  ...
			  -- ⬇️ SAFE FARM: Hook + Natural Movement + M1 Click
    if getgenv().SafeFarm then
        
        -- Step 1: Move towards titan (hover, not teleport)
        local titanHRP = currentTitanModel:FindFirstChild("HumanoidRootPart")
        local napePos = targetPart.Position
        
        -- Random approach height (human-like variation)
        local randomHeight = getgenv().AutoFarmConfig.HeightOffset + math.random(-30, 30)
        
        -- Approach position (peeche se attack)
        local approachPos
        if titanHRP then
            approachPos = (titanHRP.CFrame * CFrame.new(0, randomHeight, 35 + math.random(-10, 10))).Position
        else
            approachPos = napePos + Vector3.new(0, randomHeight, 35)
        end
        
        -- Distance check
        local dist = (root.Position - approachPos).Magnitude
        
        if dist > 15 then
            -- Phase 1: Hover towards titan (natural speed)
            local speed = getgenv().AutoFarmConfig.MoveSpeed * (0.7 + math.random() * 0.3) -- Random speed variation
            local dir = (approachPos - root.Position).Unit
            
            -- Random slight wobble in movement
            local wobble = Vector3.new(math.random(-2, 2), 0, math.random(-2, 2))
            root.AssemblyLinearVelocity = (dir + wobble).Unit * speed
            
            -- Hook animation (random chance)
            if dist < 200 and math.random(1, 5) <= 3 then -- 60% chance to hook when close
                pcall(function()
                    -- Fire hook towards titan
                    postRemote:FireServer("Hooks", "Fire", titanHRP.Position)
                end)
                task.wait(0.3 + math.random() * 0.3) -- Random hook travel time
            end
        elseif dist > 5 then
            -- Phase 2: Close range approach (slow down)
            local dir = (approachPos - root.Position).Unit
            root.AssemblyLinearVelocity = dir * (getgenv().AutoFarmConfig.MoveSpeed * 0.3)
        else
            -- Phase 3: In position - attack with M1 clicks
            root.AssemblyLinearVelocity = V3_ZERO
            
            -- Random micro-adjustments
            if math.random(1, 10) <= 4 then
                local microMove = Vector3.new(math.random(-2, 2), math.random(-1, 1), math.random(-1, 1))
                root.CFrame = root.CFrame + microMove
            end
            
            -- Attack with M1 (random timing like human)
            local dx = root.Position.X - napePos.X
            local dz = root.Position.Z - napePos.Z
            
            if (dx*dx + dz*dz) <= AttackRangeSq and (os.clock() - lastAttack) >= (0.2 + math.random() * 0.3) then
                lastAttack = os.clock()
                
                -- Random combo pattern (M1 spam with pauses)
                local comboCount = math.random(1, 3) -- Random 1-3 hits
                
                for i = 1, comboCount do
                    -- Attack
                    if slotData.Weapon == "Blades" then
                        postRemote:FireServer("Attacks", "Slash", true)
                        postRemote:FireServer("Hitboxes", "Register", napePos, math.random(500, 800))
                    else
                        pcall(function()
                            postRemote:FireServer("Spears", "S_Explode", napePos)
                        end)
                    ...
                    -- Human-like delay between combo hits
                    task.wait(0.05 + math.random() * 0.15)
                end
                
                -- Random pause after combo (breathing room)
                task.wait(0.1 + math.random() * 0.3)
            end
            
            -- Randomly switch target (not always closest)
            if #validNapes > 1 and math.random(1, 20) <= 1 then -- 5% chance
                local randomIndex = math.random(1, #validNapes)
                getgenv()._currentTargetNape = validNapes[randomIndex]
            end
        end
        
    else
        -- ⬇️ ORIGINAL MOVEMENT (unchanged)
        local titanHRP = currentTitanModel:FindFirstChild("HumanoidRootPart")
...
local function setupAutoExecute()
	if getgenv().AutoExecute and not getgenv().AutoExec then
		...
		queue_on_teleport([[
			...
			task.wait(5)
			getgenv().AutoExec = false
			loadstring(game:HttpGet("https://raw.githubusercontent.com/L-Lawliet-Hub/THUB/main/em.lua"))()
		]])
	end
end
...
FeaturesGroup:AddLabel("🛡️ Safe Farm Mode")

FeaturesGroup:AddToggle("SafeFarmToggle", {
    Text = "Safe Farm (Hook + M1 + Movement)",
    Default = false,
})
Toggles.SafeFarmToggle:OnChanged(function()
    ...
    if getgenv().SafeFarm then
        Library:Notify({
            Title = "🛡️ Safe Farm",
            Description = "Hook → Move → M1 Attack\nNatural human-like behavior!",
            Time = 5
        })
    end
end)
