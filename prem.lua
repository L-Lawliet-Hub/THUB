...
task.spawn(function()
	...
	local success, result = pcall(function()
		...
		if resp and resp.Body then
			for line in resp.Body:gmatch("[^\n]+") do
				if line:gsub("%s+", "") == PREMIUM_KEY then
					return true
				end
			end
		end
	...
	isPremium = success and result or false
	if not isPremium then
		-- Invalid key - will show notices in UI
		warn("[TITANIC HUB] Invalid premium key. Running free mode.")
	end
end)

local function premiumNotify()
	Library:Notify({
		Title = "🔒 PREMIUM FEATURE",
		Description = "This feature requires a Premium Key.\nGet yours at discord.gg/r9yDvcmW7Q",
		Time = 5
	})
end
...
local function setupAutoExecute()
	if getgenv().AutoExecute and not getgenv().AutoExec then
		...
		queue_on_teleport([[
			...
			task.wait(5)
			getgenv().AutoExec = false
			loadstring(game:HttpGet("https://raw.githubusercontent.com/L-Lawliet-Hub/THUB/main/prem.lua"))()
		]])
	end
end
